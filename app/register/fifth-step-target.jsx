...
const FifthStepTarget = () => {
  ...
  const { fullName, gender, height, weight } = params;
  const [targetWeight, setTargetWeight] = useState(59);
  ...
  const currentWeight = weight ? parseInt(weight) : 59;
  ...
  useEffect(() => {
    if (params.targetWeight) {
      // Coming back from sixth step with target weight
      const paramTargetWeight = parseInt(params.targetWeight);
      setTargetWeight(paramTargetWeight - 1);
      setHasNavigatedFromSixthStep(true);
    } else {
      // First time, set target weight to current weight - 1
      setTargetWeight(currentWeight - 1);
    }
  }, [params.targetWeight, currentWeight]);

  // Handle hardware back button - preserve target weight and go to fifth-step
  useFocusEffect(
    useCallback(() => {
      const onBackPress = () => {
        router.push({
          ...
          params: {
            ...params,
            weight: params.weight, // Preserve current weight
            targetWeight: targetWeight, // Preserve target weight
  ...
  const handleContinue = () => {
    router.push({
      ...
      params: { ...params, weight: params.weight, targetWeight: targetWeight },
    });
  };
    ...
    >
      <View style={[styles.header, { paddingTop: insets.top + 10 }]}>
        ...
        <Text style={styles.headerTitle}>
          Select Your Target <Text style={styles.highlightText}>Weight</Text>
        </Text>
      ...
      <View style={[styles.container, { paddingBottom: insets.bottom }]}>
        ...
        <View style={styles.bmiContainer}>
          <Text style={styles.bmiLabel}>Expected BMI</Text>
          ...
          <Text style={styles.bmiDescription}>
            Your target BMI will be {targetBmi} (
            {targetBmiInfo.category.toLowerCase()}). This is a{" "}
...
export default FifthStepTarget;
